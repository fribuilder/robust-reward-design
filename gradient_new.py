...
def eval_decoy_allocation(mdp, tau, decoy_value, init):
    '''
    ...
    '''
    init = init 
    _, y_opt, def_val = ent_gradient(mdp, tau, decoy_value, init, eval = False)
    return y_opt, def_val
    
def ent_gradient(mdp,tau,decoy_value,init, y_test=None,debug=False, eval=False): # add y_test
    '''
    decoy_value: 1D array, size = number of decoy spatial locations.
    This solve bounded rational attacker given the 
    '''
...
#     decoy_value_2 = extract_decoy_value(x_res_2, mdp)  # Only keep the entries corresponding to valid decoy locations.
#     print('decoy value:', decoy_value_2)

#     with open('y_res_10', 'rb') as fp:
#         y_res_2 = pickle.load(fp)
    
#     x_grad_np, y_np, def_value = ent_gradient(mdp, tau, decoy_value_2, y_test=np.array(y_res_2), debug=True)
